...
time_2019_plot <- grouped_web_over_time_2019 %>%
  ggplot(aes(x=month, y=net_sales, color=month))+geom_point(size=3, aes(group=period))+transition_states(month) +     labs(x="Month", y="Net Sales") + shadow_mark()
...
duration<- ggplot(purchase,(aes(x=avg_duration,y=total_pageviews))) + geom_jitter(height=.1) + labs(title='Page Duration vs. Total Pageviews', x='Duration', y='Pageviews')

saveRDS(duration,file='graphics/duration')
file_copy('graphics/duration', 'Milestone-6')
